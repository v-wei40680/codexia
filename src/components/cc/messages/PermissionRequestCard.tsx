...
const RESOLVED_LABEL: Record<string, string> = {
  ...
  allow_always_project: 'Always Allowed (Project)',
  deny: 'Denied',
};
  ...
  return (
    <Card className="p-4 border-amber-500/30 bg-amber-500/5 shadow-sm">
      ...
      <div className="space-y-3">
        <div className="flex flex-col gap-1.5 p-3 rounded-lg bg-background/50 border border-amber-500/20 text-xs">
        ...
        {msg.resolved ? (
          ...
          >
            {RESOLVED_LABEL[msg.resolved] ?? msg.resolved}
          </div>
        ) : (
          <div className="flex gap-2">
            ...
            >
              <Check className="w-3.5 h-3.5 mr-1" /> Always Allow
            </Button>
            <Button
              size="sm"
              variant="outline"
