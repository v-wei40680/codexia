...
export const ChatView: React.FC<ChatViewProps> = ({
  ...
  const [internalSelectedConversation, setInternalSelectedConversation] =
  ...
  const handleConversationSelect = (conversation: Conversation) => {
    ...
    console.log(
      "🔄 ChatView: Calling selectHistoryConversation",
      conversation.id,
    );
  ...
  useEffect(() => {
    if (
      currentConversationId &&
      currentConversationId.startsWith("codex-event-")
    ) {
      // Check if this is a new conversation (no messages)
      const currentConv = activeConversations.find(
  ...
  const displayedConversation =
    selectedConversation || internalSelectedConversation;
