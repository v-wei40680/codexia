...
export default function ClaudeMdPage() {
  ...
  usePageView("claude_md_editor");

  const filePath = useMemo(() => {
    if (currentFolder) {
      const trimmed = currentFolder.replace(/\/$/, "");
      return `${trimmed}/${CLAUDE_MD_FILE_NAME}`;
    }
    return CLAUDE_MD_FILE_NAME;
  }, [currentFolder]);
    ...
    (async () => {
      try {
        ...
        if (active) {
          ...
          trackEvent.errorOccurred("claude_md_file_read_failed", undefined, "claude_md_editor");
        }
      } finally {
        if (active) {
          setLoading(false);
  ...
  const handleSave = async () => {
    ...
    setError(null);
    setStatusMessage(null);
    trackEvent.fileSaved("CLAUDE.md");
    try {
      await invoke("write_file", { filePath, content });
      setStatusMessage("Changes saved.");
    } catch (err) {
      setError(getErrorMessage(err));
      trackEvent.errorOccurred("claude_md_file_save_failed", undefined, "claude_md_editor");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex items-center justify-between border-b px-6 py-4">
        <div>
          <p className="text-xs font-semibold tracking-wider text-muted-foreground">
            {currentFolder}/CLAUDE.md
          </p>
          <span className="text-sm">
            Editing project-specific Claude Code instructions
          </span>
        </div>
        <Button
          onClick={handleSave}
