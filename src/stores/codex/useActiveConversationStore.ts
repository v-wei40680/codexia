...
>()((set) => {
  // Initialize with empty cwd, will sync properly via subscription
  const initialCwd = "";
...
// Sync with CodexStore when it updates (lazy initialization)
let subscriptionInitialized = false;

export function initializeActiveConversationSubscription() {
  if (subscriptionInitialized) return;
  subscriptionInitialized = true;

  // Lazy import to avoid circular dependencies
  import("./useCodexStore").then(({ useCodexStore }) => {
    let lastCwd = "";
